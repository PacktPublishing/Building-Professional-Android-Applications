...
import com.packt.madev.portfolio.list.views.ProgressRadial;
...
public class ListAdapter extends RecyclerView.Adapter<ListAdapter.ViewHolder> {
    ...
    private float totalPortfolioValue = 1.0f;

    public void setItems(List<StockPortfolioItem> items) {
        this.list.clear();
        this.list.addAll(items);
        this.totalPortfolioValue = calculateTotalPortfolioValue(this.list);
    }

    private float calculateTotalPortfolioValue(List<StockPortfolioItem> list) {
        float valueFloat = 0.0f;
        for (StockPortfolioItem item : list) {
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        @BindView(R.id.portfolio_item_value_portion)
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        float ratio = toFloat(item.getValue()) / totalPortfolioValue;
        holder.progressRadial.setProgress(ratio);
    }
